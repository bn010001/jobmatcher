...
public class CandidateMatchService {
    ...
    public CandidateMatchService(JobRepository jobRepo, JobSwipeRepository swipeRepo, CvFileRepository cvRepo, JobService jobService) {
...
public List<MatchItemDTO> getMatches(Double lat, Double lon, Double radiusKm, Integer limit) {
    ...
    // 1) CV PARSED (obbligatorio)
    CvFile cv = cvRepo.findFirstByOwnerUsernameAndStatusOrderByUploadedAtDesc(candidate, CvProcessingStatus.PARSED)
